...
next_dsp_needs_f32(plugin_t *plugin, const ddb_waveformat_t *infmt)
{
	...
	while (ctx != NULL) {
		if (ctx->plugin->plugin.api_vminor >= 1 &&
	...
	return ctx != NULL && ctx->plugin != &g_plugin;
}
...
{
	if (plugin->child.pid < 0)
		return child_start(plugin) && process_child_is_running(plugin);
                      ...
                      ddb_waveformat_t *fmt)
{
	...
	if (!write_full(plugin->child.stdin, &request, sizeof(request)))
		return false;

	return write_full(plugin->child.stdin, writebuf, request.buffer_size);
}
                      ...
                      ddb_waveformat_t *fmt)
{
	...
	if (!read_full(plugin->child.stdout, &response, sizeof(response)))
		return false;
	...
	*frames = response.buffer_size/(fmt->bps/8)/fmt->channels;
	...
	if ((need_f32 && !(fmt->bps == 32 && fmt->is_float)) ||
		...
		if (!read_full(plugin->child.stdout, readbuf, response.buffer_size))
			return false;
		convfmt = *fmt;
		convfmt.bps = 32;
		...
		return read_full(plugin->child.stdout, (char *)samples, response.buffer_size);
	}
}
...
//	strdup((void *)((long long unsigned int)samples*(long long unsigned int)samples));
	...
	if (!process_start_child_if_not_started(plugin))
		goto failed;
